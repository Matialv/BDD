# -*- coding: utf-8 -*-
"""
Created on Wed Dec  2 11:09:02 2020
...
def info_process(disco, anio):
    ...
    xtree = ET.parse(disco + ":\Gobierno de DATOS\Tablas\DataRed\DICOSE_mejoramiento_tierras\Archivos XML\datosmejoramientostierra" + anio + ".xml")
    xroot = xtree.getroot()
    ...
    for row in xroot: 
        ...
        rows.append({"Ejercicio": Ejercicio,
                 ...
                 "MejoramientoCodigo": MejoramientoCodigo})
...
#Definimos una función para agregar las descripciones de los códigos de mejoramiento de tierras
def label_mejot (row):
    
    if row['MejoramientoCodigo'] == '1':
        return 'Praderas artificiales permanentes'
    if row['MejoramientoCodigo'] == '1a':
        return 'Hás de praderas realizadas en forma convencional'
    if row['MejoramientoCodigo'] == '2':
        return 'Siembra en cobertura'
    if row['MejoramientoCodigo'] == '2a':
        return 'Hás de praderas realizadas en siembra directa'
    if row['MejoramientoCodigo'] == '3':
        return 'Siembra a zapata'
    if row['MejoramientoCodigo'] == '4':
        return 'Campo natural fertilizado'
    if row['MejoramientoCodigo'] == '5':
        return 'Cantidad de fracciones de campo separadas entre si'

    return 'Sin datos'

def df_insertmejot(df):
    #Agrtegamos la columna 'DescripcionMejoramientoT'
    df.insert(16, 'DescripcionMejoramiento', df.apply (lambda row: label_mejot(row), axis=1))
    return(df)
...
def df_types(df):
    ...
    dataframe = df.astype({'Fecha_Ingreso_Registro': 'datetime64',
                           ...
                           'MejoramientoCodigo': 'object',
                           'DescripcionMejoramiento': 'object'}) 
...
def bdd_connection_inserts(servidor, puerto, nombre_bdd, df):
    ...
    for index, row in df.iterrows():
        cursor.execute("INSERT INTO dba.DICOSE_mejoramiento_tierras (fecha_ingreso_registro, ejercicio, departamento, descripcion_departamento, seccion_policial, area_supervision, area_enumeracion_aaee, actividad, descripcion_actividad, giro, descripcion_giro, naturaleza_juridica, descripcion_naturaleza_juridica, estrato, descripcion_estrato, mejoramiento_Codigo, descripcion_mejoramiento) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", row.Fecha_Ingreso_Registro, row.Ejercicio, row.DepartamentoCodigo, row.DescripcionDepartamento, row.SeccionalPolicialCodigo, row.AreaSupervision, row.AreaEnumeracion, row.ActividadCodigo, row.DescripcionActividad, row.GiroCodigo, row.DescripcionGiro, row.NaturalezaJuridicaCodigo, row.DescripcionNaturalezaJuridica, row.EstratoCodigo, row.DescripcionEstrato, row.MejoramientoCodigo, row.DescripcionMejoramiento)
    cnxn.commit()
...
def main():
    ...
    print('Decodificando Mejoramiento de Tierras...')
    data_frame = df_insertmejot(data_frame)
    #return(data_frame)
