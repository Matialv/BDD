# -*- coding: utf-8 -*-
"""
Created on Wed Dec  2 11:25:18 2020
...
def info_process(disco, anio):
    ...
    xtree = ET.parse(disco + ":\Gobierno de DATOS\Tablas\DataRed\DICOSE_produccion_leche\Archivos XML\datosproduccionleche" + anio + ".xml")
    xroot = xtree.getroot()
    ...
    for row in xroot: 
        ...
        TipoProduccionCodigo = row[10].text
        ...
        rows.append({"Ejercicio": Ejercicio,
                 ...
                 "TipoProduccionCodigo": TipoProduccionCodigo})
...
#Definimos una función para agregar las descripciones de los códigos de especie
def label_tpro (row):
    if row['TipoProduccionCodigo'] == '1':
        return 'Venta de LECHE CUOTA o de reparto propio'
    if row['TipoProduccionCodigo'] == '2':
        return 'Venta de leche industria'
    if row['TipoProduccionCodigo'] == '3':
        return 'Industrialización en el establecimiento'
    if row['TipoProduccionCodigo'] == '4':
        return 'Consumo humano en el establecimiento'
    if row['TipoProduccionCodigo'] == '5':
        return 'Consumo animal en el establecimiento'
    if row['TipoProduccionCodigo'] == '10':
        return 'Venta de LECHE'
    if row['TipoProduccionCodigo'] == '11':
        return 'Otros destinos'
   
    return 'Sin tipo'
def df_inserttpro(df):
    #Agrtegamos la columna 'DescripcionDepartamento'
    df.insert(18, 'DescripcionTipoProduccion', df.apply (lambda row: label_tpro(row), axis=1))
    return(df)
...
def df_types(df):
    ...
    dataframe = df.astype({'Fecha_Ingreso_Registro': 'datetime64',
                           ...
                           'TipoProduccionCodigo': 'int64',
                           'DescripcionTipoProduccion': 'object'}) 
...
def bdd_connection_inserts(servidor, puerto, nombre_bdd, df):
    ...
    for index, row in df.iterrows():
        cursor.execute("INSERT INTO dba.DICOSE_produccion_leche (fecha_ingreso_registro, ejercicio, departamento, descripcion_departamento, seccion_policial, area_supervision, area_enumeracion_aaee, actividad, descripcion_actividad, giro, descripcion_giro, naturaleza_juridica, descripcion_naturaleza_juridica, estrato, descripcion_estrato, especie, descripcion_especie, tipo_produccion, descripcion_tipo_produccion) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", row.Fecha_Ingreso_Registro, row.Ejercicio, row.DepartamentoCodigo, row.DescripcionDepartamento, row.SeccionalPolicialCodigo, row.AreaSupervision, row.AreaEnumeracion, row.ActividadCodigo, row.DescripcionActividad, row.GiroCodigo, row.DescripcionGiro, row.NaturalezaJuridicaCodigo, row.DescripcionNaturalezaJuridica, row.EstratoCodigo, row.DescripcionEstrato, row.EspecieCodigo, row.DescripcionEspecie, row.TipoProduccionCodigo, row.DescripcionTipoProduccion)
    cnxn.commit()
...
def main():
    ...
    print('Decodificando Tipo de Producción...')
    data_frame = df_inserttpro(data_frame)
