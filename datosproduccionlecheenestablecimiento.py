...
def info_process(disco, anio):
    ...
    xtree = ET.parse(disco + ":\Gobierno de DATOS\Tablas\DataRed\DICOSE_produccion_leche_establecimiento\Archivos XML\datosproduccionlecheenestablecimiento" + anio + ".xml")
    xroot = xtree.getroot()
...
#Definimos una función para agregar las descripciones de los códigos de producto
def label_prod (row):
    
    if row['ProductoCodigo'] == '6':
        return 'CREMA'
    if row['ProductoCodigo'] == '7':
        return 'MANTECA'
    if row['ProductoCodigo'] == '8':
        return 'QUESO'
    if row['ProductoCodigo'] == '9':
...
def df_insertpro(df):
    #Agrtegamos la columna 'DescripcionDepartamento'
    df.insert(18, 'DescripcionProducto', df.apply (lambda row: label_prod(row), axis=1))
...
def bdd_connection_inserts(servidor, puerto, nombre_bdd, df):
    ...
    for index, row in df.iterrows():
        cursor.execute("INSERT INTO dba.DICOSE_produccion_leche_establecimiento (fecha_ingreso_registro, ejercicio, departamento, descripcion_departamento, seccion_policial, area_supervision, area_enumeracion_aaee, actividad, descripcion_actividad, giro, descripcion_giro, naturaleza_juridica, descripcion_naturaleza_juridica, estrato, descripcion_estrato, especie, descripcion_especie, producto_codigo, descripcion_producto) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", row.Fecha_Ingreso_Registro, row.Ejercicio, row.DepartamentoCodigo, row.DescripcionDepartamento, row.SeccionalPolicialCodigo, row.AreaSupervision, row.AreaEnumeracion, row.ActividadCodigo, row.DescripcionActividad, row.GiroCodigo, row.DescripcionGiro, row.NaturalezaJuridicaCodigo, row.DescripcionNaturalezaJuridica, row.EstratoCodigo, row.DescripcionEstrato, row.EspecieCodigo, row.DescripcionEspecie, row.ProductoCodigo, row.DescripcionProducto)
    cnxn.commit()
...
def main():
    ...
    data_frame = df_insertpro(data_frame)
