# -*- coding: utf-8 -*-
"""
Created on Wed Dec  2 10:47:49 2020
...
def info_process(disco, anio):
    ...
    xtree = ET.parse(disco + ":\Gobierno de DATOS\Tablas\DataRed\DICOSE_tenencia_tierras\Archivos XML\datostenenciastierra" + anio + ".xml")
    xroot = xtree.getroot()
...
#Definimos una función para agregar las descripciones de los códigos de tenencia de tierras
def label_ttierra (row):
    if row['TenenciaCodigo'] == '1':
        return 'Propiedad de la Empresa'
    if row['TenenciaCodigo'] == '2':
        return 'Arrendada'
    if row['TenenciaCodigo'] == '3':
        return 'Pastoreo a 11 meses o régimen similar'
    if row['TenenciaCodigo'] == '4':
        return 'Ocupada por la Empresa y socios integrantes'
    if row['TenenciaCodigo'] == '5':
        return 'Medianeria'
    if row['TenenciaCodigo'] == '6':
        return 'Ocupada a cualquier otro titulo'

    return 'Sin estrato'

def df_inserttierra (df):
    #Agrtegamos la columna 'DescripcionDepartamento'
    df.insert(16, 'DescripcionTenencia', df.apply (lambda row: label_ttierra(row), axis=1))
    return(df)
...
def df_types(df):
    ...
    dataframe = df.astype({'Fecha_Ingreso_Registro': 'datetime64',
                           ...
                           'TenenciaCodigo': 'int64',
                           'DescripcionTenencia': 'object'}) 
...
def bdd_connection_inserts(servidor, puerto, nombre_bdd, df):
    ...
    for index, row in df.iterrows():
        cursor.execute("INSERT INTO dba.DICOSE_tenencia_tierras (fecha_ingreso_registro, ejercicio, departamento, descripcion_departamento, seccion_policial, area_supervision, area_enumeracion_aaee, actividad, descripcion_actividad, giro, descripcion_giro, naturaleza_juridica, descripcion_naturaleza_juridica, estrato, descripcion_estrato, tenencia_codigo, descripcion_tenencia) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", row.Fecha_Ingreso_Registro, row.Ejercicio, row.DepartamentoCodigo, row.DescripcionDepartamento, row.SeccionalPolicialCodigo, row.AreaSupervision, row.AreaEnumeracion, row.ActividadCodigo, row.DescripcionActividad, row.GiroCodigo, row.DescripcionGiro, row.NaturalezaJuridicaCodigo, row.DescripcionNaturalezaJuridica, row.EstratoCodigo, row.DescripcionEstrato, row.TenenciaCodigo, row.DescripcionTenencia)
    cnxn.commit()
...
def main():
    ...
    print('Decodificando Tipo de Tierra...')
    data_frame = df_inserttierra(data_frame)
