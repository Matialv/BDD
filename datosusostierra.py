# -*- coding: utf-8 -*-
"""
Created on Wed Dec  2 11:00:13 2020
...
def info_process(disco, anio):
    ...
    xtree = ET.parse(disco + ":\Gobierno de DATOS\Tablas\DataRed\DICOSE_usos_tierra\Archivos XML\datosusostierra" + anio + ".xml")
    xroot = xtree.getroot()
...
def df_insertest(df):
    ...
    return(df)
#Definimos una función para agregar las descripciones de los códigos de usos de tierras
def label_usos (row):
    if row['UsoCodigo'] == '1':
        return 'Praderas artificiales permanentes'
    if row['UsoCodigo'] == '2':
        return 'Campo mejorado'
    if row['UsoCodigo'] == '3':
        return 'Campo fertilizado'
    if row['UsoCodigo'] == '4':
        return 'Cultivos forrajeros anuales'
    if row['UsoCodigo'] == '5':
        return 'Huertas, frutales y viñedos'
    if row['UsoCodigo'] == '6':
        return 'Tierras de labranza'
    if row['UsoCodigo'] == '7':
        return 'Montes artificiales (forestación)'
    if row['UsoCodigo'] == '8':
        return 'Campo natural y rastrojos'

    return 'Sin estrato'
def df_insertuso(df):
    #Agrtegamos la columna 'DescripcionDepartamento'
    df.insert(16, 'DescripcionUsos', df.apply (lambda row: label_usos(row), axis=1))
    return(df)
...
def bdd_connection_inserts(servidor, puerto, nombre_bdd, df):
    ...
    for index, row in df.iterrows():
        cursor.execute("INSERT INTO dba.DICOSE_usos_tierra (fecha_ingreso_registro, ejercicio, departamento, descripcion_departamento, seccion_policial, area_supervision, area_enumeracion_aaee, actividad, descripcion_actividad, giro, descripcion_giro, naturaleza_juridica, descripcion_naturaleza_juridica, estrato, descripcion_estrato, usos, descripcion_usos) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", row.Fecha_Ingreso_Registro, row.Ejercicio, row.DepartamentoCodigo, row.DescripcionDepartamento, row.SeccionalPolicialCodigo, row.AreaSupervision, row.AreaEnumeracion, row.ActividadCodigo, row.DescripcionActividad, row.GiroCodigo, row.DescripcionGiro, row.NaturalezaJuridicaCodigo, row.DescripcionNaturalezaJuridica, row.EstratoCodigo, row.DescripcionEstrato, row.UsoCodigo, row.DescripcionUsos)
    cnxn.commit()
...
def main():
    ...
    print('Decodificando uso de Tierras...')
    data_frame = df_insertuso(data_frame)
